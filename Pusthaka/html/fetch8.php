...
$columns = array('cid','bid','access_no','barcode','lending_type','copy_status','title','authors','edition','publisher','published_year','isbn');
...
///***********************************************************************************************************************//////////////////////////
     ...
     $query="SELECT copy.cid,copy.bid ,copy.access_no,copy.barcode,copy.Availability,copy.copy_status,book.title, book.authors,book.edition,book.publisher,book.published_year,book.isbn FROM  copy LEFT JOIN book ON copy.bid=book.bid WHERE " ; 
...

{
 ...
 $sub_array[] = $row["barcode"];
  $sub_array[] = $row["Availability"];
   $sub_array[] = $row["copy_status"];


 $sub_array[] = $row["title"];

 $sub_array[] = $row["authors"];
  $sub_array[] = $row["edition"];
   $sub_array[] = $row["publisher"];
  $sub_array[] = $row["published_year"];
...
function get_all_data($connect)
{
  //$query = "SELECT lid,member,copy,loaned_by, CAST(date_loaned AS DATE)  FROM loan";
    ...
    // $query="SELECT copy.cid,copy.bid ,copy.access_no,book.title, book.authors,book.isbn FROM  copy LEFT JOIN book ON copy.bid=book.bid  WHERE copy.copy_status='OK' AND cid  NOT IN  (SELECT cid FROM  copy_check  WHERE name= $year) "; 
//***********************************************************************************************************************/////////////////////     


$query="SELECT copy.cid,copy.bid ,copy.access_no,copy.barcode,copy.Availability,copy.copy_status,book.title, book.authors,book.edition,book.publisher,book.published_year,book.isbn FROM  copy LEFT JOIN book ON copy.bid=book.bid ";
