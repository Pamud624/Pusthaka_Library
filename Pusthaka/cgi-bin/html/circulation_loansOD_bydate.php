<?php
	...
	$PageTitle = "Books Due Today";
	include('../inc/init.php');
	

	// Calculate $noOverdue
	$sql = sprintf("SELECT count(lid) FROM loan WHERE returned=0 AND date_due = '%s'", date("Y-m-d"));
	$rs = executeSqlQuery($sql);
	$r = mysqli_fetch_array($rs);
	$noOverdue = $r[0];

	// Retrieve the set of members who have overdue books ($rsMembers)
	$sql = sprintf("SELECT distinct(member) AS mid FROM loan WHERE returned=0 AND date_due = '%s'", date("Y-m-d"));
